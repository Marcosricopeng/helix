...
public class TestZkClientAsyncFailure extends ZkTestBase {
  ...
  private ObjectName buildObjectName(String tag, String key, String instance) throws
  ...
  public void testAsyncWrite() throws JMException {
    TestZkClientAsyncFailure.MockAsyncZkClient
        testZkClient = new TestZkClientAsyncFailure.MockAsyncZkClient(_zkServerAddress);
    try {
      ...
      Assert.assertEquals(((ZNRecord) testZkClient.readData(NODE_PATH)).getSimpleField("test"),
          "data");
      Assert.assertEquals((long) _beanServer.getAttribute(instancesName, ZkClientPathMonitor.PredefinedMetricDomains.WriteFailureCounter.toString()), 0);
      ...
      Assert.assertEquals((long) _beanServer.getAttribute(instancesName, ZkClientPathMonitor.PredefinedMetricDomains.WriteFailureCounter.toString()), 1);
    } finally {
      testZkClient.close();
  ...
  public void testAsyncRead() throws JMException {
    ...
    try {
      ...
      Assert.assertEquals((long) _beanServer.getAttribute(instancesName, ZkClientPathMonitor.PredefinedMetricDomains.ReadFailureCounter.toString()), 0);
      ...
      Assert.assertEquals((long) _beanServer.getAttribute(instancesName, ZkClientPathMonitor.PredefinedMetricDomains.ReadFailureCounter.toString()), 1);
    } finally {
      testZkClient.setAsyncCallRC(KeeperException.Code.OK.intValue());
      testZkClient.close();
      _zkClient.delete(NODE_PATH);
